...
import { summarizeEntities } from "../../home-assistant/entities";
import { getFirstChatResponse } from "../../openai/response";
import { log } from "../logs";
import { HOME_ASSISTANT_SUMMARIES_DEVICE_ENTITY_PREFIX } from "../../config";

// TODO: Move this to a dedicated assistant
...
export function getPromptForDeviceSummary(states, deviceName, paramOptions) {
  ...
  const deviceEntities = states
    ...
    .filter(
      (el) =>
        !el.entity_id.includes(HOME_ASSISTANT_SUMMARIES_DEVICE_ENTITY_PREFIX),
    ); // Ignoring ourselves
  ...
  const briefing = `
    ...
    You must filter out the noise and respond with a single sentence, no bullets points summary of maximum ${
      options.length
    } characters. 
    ...
    CONTEXT :
    For relative date calculation, today is ${new Date().toLocaleDateString(
      "en-US",
      {
        weekday: "long",
        year: "numeric",
 ...
 * @param {Array} states - List of states of all entities
 * @param {string} deviceName - Name of the device to summarize
 * @param {Object} [paramOptions] - Optional options for the prompt
 * @param {number} paramOptions.length - Maximum length of the prompt
 * @param {string} paramOptions.customPrompt - Custom prompt to add to the briefing
...
export async function updateDeviceSummary(
  ...
  states: any[],
  deviceName: string,
  paramOptions?: { length: number; customPrompt: string },
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
): Promise<any> {
  ...
  return ha.states.update(
    "sensor",
    `${HOME_ASSISTANT_SUMMARIES_DEVICE_ENTITY_PREFIX}${deviceName}`,
    {
      state: 1,
      attributes: {
        content: summary,
